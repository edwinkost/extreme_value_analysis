...
def get_return_period_gumbel(p_zero_in_pcraster, loc_in_pcraster, scale_in_pcraster, flvol_in_pcraster, max_return_period = 1e9):
    ...
    p_zero  = np.float64(pcr.pcr2numpy(p_zero_in_pcraster, vos.MV))
    loc     = np.float64(pcr.pcr2numpy(loc_in_pcraster   , vos.MV))
    scale   = np.float64(pcr.pcr2numpy(scale_in_pcraster , vos.MV))
    flvol   = np.float64(pcr.pcr2numpy(flvol_in_pcraster , vos.MV))
